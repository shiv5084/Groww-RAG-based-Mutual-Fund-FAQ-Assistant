...
export default function HomePage() {
  ...
  // Send message (creates thread lazily on first send)
  const sendMessage = async (message: string) => {
    if (isLoading) return;
    let thread = currentThread;
    if (!thread) {
      thread = await apiClient.createThread();
      setCurrentThread(thread);
    }
    ...
    try {
      ...
      const userMessage: Message = {
        id: `temp-${Date.now()}`,
        thread_id: thread.thread_id,  // Use the thread variable, not currentThread
        role: 'user',
        content: message,
      ...
      const response: AssistantResponse = await apiClient.sendMessage(
        thread.thread_id,  // Use the thread variable, not currentThread
      ...
      const assistantMessage: Message = {
        ...
        thread_id: thread.thread_id,  // Use thread variable, not currentThread
        role: 'assistant',
      ...
      setError(err instanceof ApiError ? err.message : `Failed to send message: ${err}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExampleQuestion = async (question: string) => {
    if (!currentThread) {
      // Create thread first, then set the question
      const thread = await apiClient.createThread();
      setCurrentThread(thread);
      setMessages([]);
    }
  ...
  // Thread is created lazily on first message send instead of blocking UI on mount
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        {/* Input Form - Fixed at Bottom - Always Visible */}
